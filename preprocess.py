...
def main():
    # calc_first_entropy_honke(pd.read_csv('./data/words_with_zero.csv', index_col=0))
...
def make_second_guess_list(first_guess : str):
    ...
    for i in range(3):
        for j in range(3):
            for k in range(3):
                for l in range(3):
                    for m in range(3):
                        entropies = pd.read_csv('./data/words_with_zero.csv', index_col=0)
    ...
    second_guesses.to_csv('./outputs/second_guesses.csv')
...
def calc_first_entropy_all(entropies : pd.DataFrame):
    ...
    entropies.to_csv('./outputs/init_entropies.csv')

# すべての入力可能単語を本家の解答候補と総当たりしてエントロピーの初期値を計算する
def calc_first_entropy_honke(entropies : pd.DataFrame):
    ans_can = list(pd.read_csv('./data/candidates.csv', header=None)[0])
    # 総当りで各状態の出現回数を求める
    for word_num in range(word_list_len):
        # 各状態ごとの出現回数を記録する5次元配列
        count_each_judge = np.zeros((3,3,3,3,3), dtype= int)
                ...
                for k in range(3):
                    for l in range(3):
                        for m in range(3):
                            ...
                            if count_each_judge[i][j][k][l][m] > 0: # == 0 の場合は確率も0なので自己エントロピーも0
                                entropies.iat[word_num, 0] -= (count_each_judge[i][j][k][l][m] / word_list_len) * math.log2(count_each_judge[i][j][k][l][m] / word_list_len)
    entropies.to_csv('./outputs/honke_init_entropies_with_count_each_judge.csv')                   
